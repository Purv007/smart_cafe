...
const Menu = () => {
  ...
  const loadWishlist = async (forceRefresh = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Safely map only valid wishlist items with a product reference
  ...
  const toggleWishlist = async (productId) => {
    ...
    try {
      ...
      // Persist cache immediately for better resilience across revisits
      localStorage.setItem(`wishlist_${token}`, JSON.stringify(nextWishlist));
      localStorage.setItem(`wishlist_timestamp_${token}`, Date.now().toString());
      ...
      if (isCurrentlyInWishlist) {
        ...
        if (response.ok) {
          ...
          if (!reverted.includes(productId)) reverted.push(productId);
          setWishlist(reverted);
          localStorage.setItem(`wishlist_${token}`, JSON.stringify(reverted));
          localStorage.setItem(`wishlist_timestamp_${token}`, Date.now().toString());
        ...
        if (response.ok) {
          ...
          // Revert optimistic update on error
          const reverted = wishlist.filter(id => id !== productId);
          setWishlist(reverted);
          localStorage.setItem(`wishlist_${token}`, JSON.stringify(reverted));
          localStorage.setItem(`wishlist_timestamp_${token}`, Date.now().toString());
          let errorData;
